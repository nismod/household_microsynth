...
class ReferencePerson:
    ...
    def __add_ref_persons(self, area, constraints):
        ...
        if m4605_sum != m4201_sum:
            ...
            # print(m4605)
            m4605_adj = humanleague.qisi(m4605.astype(float), [np.array([0]), np.array([1])],
                                         [nssec_4605_adj, tenure_4201])
        ...
        pop = humanleague.qis([np.array([0, 1]), np.array([2, 1]), np.array([3]), np.array([4])],
                              [m4605, m4201, mq111, m1102])
