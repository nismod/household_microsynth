...
def check_hh(msynth, total_occ_dwellings, total_households, total_communal, total_household_poplb, total_communal_pop,
    ...
    assert np.array_equal(sorted(msynth.dwellings.LC4402_C_TYPACCOM.unique()),
                          np.insert(msynth.type_index, 0, [msynth.NOTAPPLICABLE]))
    # contains unk+na
    assert np.array_equal(sorted(msynth.dwellings.LC4402_C_TENHUK11.unique()),
                          np.insert(msynth.tenure_index, 0, [msynth.NOTAPPLICABLE, msynth.UNKNOWN]))
    assert np.array_equal(sorted(msynth.dwellings.LC4408_C_AHTHUK11.unique()),
                          np.insert(msynth.comp_index, 0, [msynth.UNKNOWN]))
    ...
    if not scotland:
        assert msynth.dwellings[
                   msynth.dwellings.QS420_CELL != msynth.NOTAPPLICABLE].CommunalSize.sum() == total_communal_pop
    ...
    for i in msynth.type_index:
        assert len(msynth.dwellings[(msynth.dwellings.LC4402_C_TYPACCOM == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)]) == sum(
            msynth.lc4402[msynth.lc4402.C_TYPACCOM == i].OBS_VALUE)

    # Tenure (occupied only)
    for i in msynth.tenure_index:
        assert len(msynth.dwellings[(msynth.dwellings.LC4402_C_TENHUK11 == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)]) == sum(
            msynth.lc4402[msynth.lc4402.C_TENHUK11 == i].OBS_VALUE)
    ...
    for i in msynth.ch_index:
        assert len(msynth.dwellings[(msynth.dwellings.LC4402_C_CENHEATHUK11 == i)
                                    ...
                                    & (msynth.dwellings.QS420_CELL == msynth.NOTAPPLICABLE)]) == sum(
            msynth.lc4402[msynth.lc4402.C_CENHEATHUK11 == i].OBS_VALUE)
    ...
    for i in msynth.comp_index:
        assert len(msynth.dwellings[(msynth.dwellings.LC4408_C_AHTHUK11 == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)
                                    & (msynth.dwellings.QS420_CELL == msynth.NOTAPPLICABLE)]), sum(
            msynth.lc4408[msynth.lc4408.C_AHTHUK11 == i].OBS_VALUE)

    # Rooms (ignoring communal and unoccupied)
    assert np.array_equal(
        sorted(msynth.dwellings[msynth.dwellings.LC4402_C_TYPACCOM != msynth.NOTAPPLICABLE].LC4404_C_ROOMS.unique()),
        msynth.lc4404["C_ROOMS"].unique())
    for i in msynth.lc4404["C_ROOMS"].unique():
        assert len(msynth.dwellings[(msynth.dwellings.LC4404_C_ROOMS == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)
                                    & (msynth.dwellings.QS420_CELL == msynth.NOTAPPLICABLE)]) == sum(
            msynth.lc4404[msynth.lc4404.C_ROOMS == i].OBS_VALUE)
    # check communal residences rooms are all UNKNOWN
    assert (msynth.dwellings[
                msynth.dwellings.CommunalSize != msynth.NOTAPPLICABLE].LC4404_C_ROOMS.unique() == msynth.UNKNOWN)
    # == msynth.UNKNOWN)
    ...
    assert np.array_equal(sorted(
        msynth.dwellings[msynth.dwellings.LC4402_C_TYPACCOM != msynth.NOTAPPLICABLE].LC4405EW_C_BEDROOMS.unique()),
                          msynth.lc4405["C_BEDROOMS"].unique())
    for i in msynth.lc4405["C_BEDROOMS"].unique():
        assert len(msynth.dwellings[(msynth.dwellings.LC4405EW_C_BEDROOMS == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)
                                    & (msynth.dwellings.QS420_CELL == msynth.NOTAPPLICABLE)]) == sum(
            msynth.lc4405[msynth.lc4405.C_BEDROOMS == i].OBS_VALUE)
    # check communal residences bedrooms are all UNKNOWN
    assert (msynth.dwellings[
                msynth.dwellings.CommunalSize != msynth.NOTAPPLICABLE].LC4405EW_C_BEDROOMS.unique() == msynth.UNKNOWN)
    ...
    assert np.array_equal(
        sorted(msynth.dwellings[msynth.dwellings.LC4605_C_NSSEC != msynth.UNKNOWN].LC4605_C_NSSEC.unique()),
        msynth.lc4605["C_NSSEC"].unique())
    for i in msynth.lc4605["C_NSSEC"].unique():
        assert len(msynth.dwellings[(msynth.dwellings.LC4605_C_NSSEC == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)
                                    & (msynth.dwellings.QS420_CELL == msynth.NOTAPPLICABLE)]) >= sum(
            msynth.lc4605[msynth.lc4605["C_NSSEC"] == i].OBS_VALUE)
    ...
    assert np.array_equal(
        sorted(msynth.dwellings[msynth.dwellings.LC4202_C_ETHHUK11 != msynth.UNKNOWN].LC4202_C_ETHHUK11.unique()),
    ...
    for i in msynth.lc4202["C_ETHHUK11"].unique():
        assert len(msynth.dwellings[(msynth.dwellings.LC4202_C_ETHHUK11 == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)
                                    & (msynth.dwellings.QS420_CELL == msynth.NOTAPPLICABLE)]) == sum(
            msynth.lc4202[msynth.lc4202["C_ETHHUK11"] == i].OBS_VALUE)

    # Cars (ignoring communal and unoccupied)
    assert np.array_equal(
        sorted(msynth.dwellings[msynth.dwellings.LC4202_C_CARSNO != msynth.UNKNOWN].LC4202_C_CARSNO.unique()),
        msynth.lc4202["C_CARSNO"].unique())
    for i in msynth.lc4202["C_CARSNO"].unique():
        assert len(msynth.dwellings[(msynth.dwellings.LC4202_C_CARSNO == i)
                                    & (msynth.dwellings.LC4404_C_SIZHUK11 != 0)
                                    & (msynth.dwellings.QS420_CELL == msynth.NOTAPPLICABLE)]) == sum(
            msynth.lc4202[msynth.lc4202["C_CARSNO"] == i].OBS_VALUE)
...
def check_hrp(msynth, total_hrps):
    ...
    for area in areas:
        if len(msynth.hrps[msynth.hrps.Area == area]) != sum(
                msynth.lc4201[msynth.lc4201.GEOGRAPHY_CODE == area].OBS_VALUE):
    ...
    for tenure in tenures:
        if len(msynth.hrps[msynth.hrps.LC4605_C_TENHUK11 == tenure]) != sum(
                msynth.lc4201[msynth.lc4201.C_TENHUK11 == tenure].OBS_VALUE):
    ...
    for eth in eths:
        if len(msynth.hrps[msynth.hrps.LC4201_C_ETHPUK11 == eth]) != sum(
                msynth.lc4201[msynth.lc4201.C_ETHPUK11 == eth].OBS_VALUE):
    ...
    for lifestage in lifestages:
        if len(msynth.hrps[msynth.hrps.QS111_C_HHLSHUK11 == lifestage]) != sum(
                msynth.qs111[msynth.qs111.C_HHLSHUK11 == lifestage].OBS_VALUE):
    ...
    for livarr in livarrs:
        if len(msynth.hrps[msynth.hrps.LC1102_C_LARPUK11 == livarr]) != sum(
                msynth.lc1102[msynth.lc1102.C_LARPUK11 == livarr].OBS_VALUE):
